...
type cpRecord struct {  //compaction record for one level
	level int
	ikey  internalKey    //起始的key
}

type atRecord struct {   //add file record
	level int            //所在level
	num   int64          //file num
	size  int64          //file size
	imin  internalKey    //最小key
	imax  internalKey    //最大key
}

type dtRecord struct {   //delete file record
