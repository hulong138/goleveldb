...
type vDelta struct {
	vid     int64      //session.stVersion.id
	added   []int64    // added file id
	deleted []int64    // deleted file id
}
...
func (s *session) refLoop() {
	var (
		fileRef    = make(map[int64]int)    // Table file reference counter  ,文件id-->引用计数
		ref        = make(map[int64]*vTask) // Current referencing version store   版本号-->vTask
		deltas     = make(map[int64]*vDelta)  // 版本号-->vDelta
		referenced = make(map[int64]struct{})  //表示 版本号ref已经处理过了， key是版本号
		released   = make(map[int64]*vDelta)  // Released version that waiting for processing   已经释放的版本号-->vDelta
		abandoned  = make(map[int64]struct{}) // Abandoned version id；；     key是版本号
		next, last int64   //版本号
	)
...
func (s *session) newManifest(rec *sessionRecord, v *version) (err error) {
	...
	s.fillRecord(rec, true)  //把session的信息填充到rec中
	v.fillRecord(rec)                 //把version中的table信息填充到rec中
