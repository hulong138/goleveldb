...
type DB struct {
	...
	// Write.
	batchPool    sync.Pool
	writeMergeC  chan writeMerge   //write merge channel
	writeMergedC chan bool         //write merged channel
	writeLockC   chan struct{}     //write lock channel
	writeAckC    chan error        //write ack channel
	writeDelay   time.Duration
	writeDelayN  int
	tr           *Transaction

	// Compaction.
	compCommitLk     sync.Mutex             //compaction commit lock
	tcompCmdC        chan cCmd              //tfile compaction command channel
	tcompPauseC      chan chan<- struct{}   //tfile compaction pause channel
	mcompCmdC        chan cCmd              //memory compaction command channel
	compErrC         chan error             //compaction error  channel
	compPerErrC      chan error             //compaction permanent error channel
	compErrSetC      chan error             //compaction error set channel
	compWriteLocking bool
	compStats        cStats                 //compaction stats
	memdbMaxLevel    int // For testing.

	// Close.
	closeW sync.WaitGroup       //close wait gourp
