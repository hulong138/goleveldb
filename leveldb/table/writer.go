...
type blockWriter struct {
	restartInterval int       //重启点间隔
	buf             util.Buffer //存放编码后的kv数据
	nEntries        int       //kv对数量
	prevKey         []byte    //上一个key
	restarts        []uint32  //重启点offset
	scratch         []byte    //临时编码buf
}
//添加一对kv到block
...
//结束一个block快，添加重启点的offset和数量到buf中。
...
type filterWriter struct {
	...
	buf       util.Buffer
	nKeys     int       //key的数量
	offsets   []uint32 //filter data块在block中的偏移量
	baseLg    uint     //默认值为11，表示每2KB的数据，创建一个新的过滤器来存放过滤数据。
}
...
//结束过滤器block: 生成过滤器数据块，在filter date的后面添加filter offset信息
func (w *filterWriter) finish() error {
	...
	w.offsets = append(w.offsets, uint32(w.buf.Len())) //add filter offset's offset
...
//生成过滤器内容：添加偏移量，通过key的hash值生成内容到过滤器buf中
...
//把buf中的数据压缩，计算crc，写到writer的输出流中，修改offset，返回block handle
...
//把上一个结束的block的block handle当成value写到indexBlock中，key取大于或等于上一个结束block的key，小于输入的key
...
//结束一个datablock，添加重启点的offset和数量到buf中
//datablock中的数据压缩，写到writer中，修改offset，返回block handle
//生成当前offset的过滤器数据到过滤器buf中
