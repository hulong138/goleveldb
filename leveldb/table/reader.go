...
type block struct {
	...
	bh             blockHandle
	data           []byte
	restartsLen    int   //重启点个数
	restartsOffset int   //重启点offset
}
//在block中找到比key要大的重启点的上一个重启点index和offset
...
//迭代器定位到比key大或相等的一个数据项，如果找不到，则返回false
...
type filterBlock struct {
	bpool      *util.BufferPool
	data       []byte   //过滤器数据块
	oOffset    int     //offset's offset
	baseLg     uint
	filtersNum int      //过滤器个数
}
//入参offset指datablock在文件中的偏移
func (b *filterBlock) contains(filter filter.Filter, offset uint64, key []byte) bool {
	i := int(offset >> b.baseLg)
	if i < b.filtersNum {
		o := b.data[b.oOffset+i*4:]
		n := int(binary.LittleEndian.Uint32(o))      // current offset
		m := int(binary.LittleEndian.Uint32(o[4:]))  //next offset
		if n < m && m <= b.oOffset {
...
//从底层的文件reader读取数据块，校验crc正确，解压缩
//原始数据格式：data，compress，crc
...
func (r *Reader) readBlock(bh blockHandle, verifyChecksum bool) (*block, error) {
	...
	restartsLen := int(binary.LittleEndian.Uint32(data[len(data)-4:])) //重启点个数
	b := &block{
		bpool:          r.bpool,
		bh:             bh,
		data:           data,
...
//优先从缓存中读取Block块，如果缓存中不存在，则从底层文件读取，并填充缓存
...
//从缓存中读取过滤器Block并解析内容，如果缓存中不存在，则从底层文件读取，并填充缓存
...
//根据blockHandle, 读取数据Block，根据切片范围，构造一个数据Block的迭代器
