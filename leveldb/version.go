...
type version struct {
	...
	cSeek unsafe.Pointer         //指向tSet结构指针
...
//遍历和ikey有交叠的aux文件和当前版本的sst文件，如果f 或者lf返回false则提前结束遍历
...
func (v *version) get(aux tFiles, ikey internalKey, ro *opt.ReadOptions, noValue bool) (value []byte, tcomp bool, err error) {
	...
	v.walkOverlapping(aux, ikey, func(level int, t *tFile) bool {
		//key没有找到，返回true，需要继续找
		//key找到了，或者出错了，返回false，退出查找。
		...
		case nil:
		case ErrNotFound:
			return true     //key没有找到，返回true，需要继续找
		default:
			err = ferr
			return false  //出错了，返回false，退出查找。
		}
		...
		if fukey, fseq, fkt, fkerr := parseInternalKey(fikey); fkerr == nil {
			if v.s.icmp.uCompare(ukey, fukey) == 0 {
				...
				if level <= 0 {
					if fseq >= zseq { //找到多个不同的版本，用序号大的替换序号小的
						zfound = true
						zseq = fseq
						zkt = fkt
						zval = fval
					...
					return false  //找到了，不继续查找
				}
			}
		} else {
			err = fkerr
			return false     //出错了，不继续查找
		}
